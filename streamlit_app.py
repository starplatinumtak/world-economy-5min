...
st.set_page_config(page_title="世界経済5分チェック 第4.7版", page_icon="🌎", layout="wide")
...
for c,(k,_) in zip(cols,symbols.items()):
    v,p=stats(M[k])
    if v is None:c.metric(k,"取得不可","—")
    else:c.metric(k,f"{v/10:.2f}%" if k=="米10年債" else fmt(v,0 if k=="日経平均" else 2),pct(p))
...
def get_boj():
    try:
        r=requests.get(BOJ_URL,params={"format":"json","lang":"jp","db":"FM","code":"FM01'STRDCLUCON"},timeout=20)
        r.raise_for_status()
        return r.json()
...
if pairs:
    ld,lv=pairs[-1];pv=pairs[-2][1] if len(pairs)>1 else None
    a,b,c=st.columns(3)
...
else:st.warning("日銀APIから現在値を取得できませんでした。")
...
st.divider();st.header("📅 今後の重要経済指標")
...
st.info("第4.7版は実績・公式発表予定を自動取得。市場コンセンサス予想の履歴は、信頼できる予想データ源を接続した段階で自動計算します。")
...
st.divider();st.header("🧠 今日の相場の因果関係")
...
if OPENAI_KEY and st.button("AI分析を更新",type="primary"):
    data={"市場":{k:stats(v) for k,v in M.items()},"米国":{"CPI":CPI,"失業率":UNEMP,"雇用":PAY,"平均時給":WAGE,"PPI":PPI},"日本":{"日銀O/N":pairs[-1] if pairs else None},"ニュース":[e for _,_,e in sorted(items,key=lambda z:z[0],reverse=True)[:10]]}
